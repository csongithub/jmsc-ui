<template>
    <div v-if="party_id  !== null" class="q-gutter-sm">
        <div class="q-mb-sm">
            <q-radio v-model="selected_tab"  val="exists" label="Existing" />
            <q-radio v-model="selected_tab"  val="new" label="New Account" />
        </div>
        <q-separator/>
        <div v-if="selected_tab === 'exists'">
            <div class="row q-mt-sm">
              <div class="col-4">
                Enter Account ID & Press TAB or ENTER
              </div>
              <div class="col-2">
            ...
            <div class="row q-mb-sm">
              <q-input type="number" @keydown.tab.prevent="fetchAccount($event.target.value)" @keydown.enter.prevent="fetchAccount($event.target.value)"
                        @blur="fetchAccount($event.target.value)" dense outlined  v-model="fetch_account_id" placeholder="Fetch & Add Account"/>
            </div>
            ...
            <div v-if="fetched_account !== null" class="row q-mt-xs">
                ...
                <div class="col-1">
                    <q-btn size="sm" label="add" color="primary" @click="linkAccount(fetched_account.id)"></q-btn>
                </div>
            </div>
            <q-table class="my-sticky-header-table q-mt-lg"
                ...
                <template v-slot:body-cell-actions="props">
                    <q-btn class="text-capitalize" color="red" flat dense size="xs" :icon="icons.delete" @click="deLink(props.row)">
                        <q-tooltip>Remove this account from party</q-tooltip>
                    </q-btn>
                </template>
            </q-table>
        </div>
        <div v-else-if="selected_tab==='new'">
            <q-form @submit="addPartyAccount" @reset="reset" class="q-gutter-md q-mt-sm">
              <div class="row">
                <div class="col">
                  <q-input
                    dense
              ...
              <div class="row">
                <div class="col">
                  <q-input
                    ...
                    full-width
                    lazy-rules
                    :rules="[val => (val && val.length > 0) || 'bank Name']"
                ...
                <div class="col q-ml-sm">
                  <q-input
                    ...
                    full-width
                    lazy-rules
                    :rules="[val => (val && val.length > 0) || 'Branch Name']"
                ...
                <div class="col q-ml-sm">
                  <q-input
                    ...
                    full-width
                    lazy-rules
                    :rules="[val => (val && val.length > 0) || 'Branch Code']"
...
export default {
  ...
  methods: {
    ...
    getPartyAccounts() {
        ...
        PartyService.getPartyLinkedAccounts(this.client_id, this.party_id).then(response => {
    ...
    linkAccount(account_id) {
        PartyAccountService.linkPartyAccount(this.client_id, this.party_id, account_id)
            .then((response) => {
            if(response === 0) {
                this.getPartyAccounts()
            } else if(response === -1) {
    ...
    deLink(account) {
        PartyAccountService.removeAccount(this.client_id, this.party_id, account.id)
            .then((response) => {
            if(response === 0) {
                this.getPartyAccounts()
            } 
    ...
    addPartyAccount() {
      ...
      PartyAccountService.addPartyAccount(this.account).then(response => {
    ...
    getBankDetails(){
      ...
      PartyAccountService.bankByIFSC(this.account.ifscCode).then(response => {
        if(response !== "-1"){
