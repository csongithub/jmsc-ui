      ...
      >
        ...
        <template v-slot:top-left>
         <q-btn class="q-mt-sm q-mr-sm text-capitalize" 
                ...
                size="sm"
                glossy
                @click="!showClosed ? getAllActiveBankGuarantee() : getAllClosedBankGuarantee()"/>
          <q-checkbox class="q-mr-sm" v-model="showClosed" label="Show Closed" color="primary" @click="toggelShowAll()">
            <q-tooltip>Show all closed/terminated BG</q-tooltip>
          </q-checkbox>
        </template>
...
export default {
  ...
  created() {},
  mounted() {
    this.getAllActiveBankGuarantee()
  },
  data() {
    return {
  ...
  methods: {
    ...
    toggelShowAll(){
      if(!this.showClosed){
        ...
        this.getAllClosedBankGuarantee()
      }
    },
    getAllActiveBankGuarantee() {
      this.loading = true;
      CreditFacilityService.getAllActiveFacilitiesByType(this.clientId, 'BG')
        .then(response => {
    ...
    getAllClosedBankGuarantee() {
      this.loading = true;
      CreditFacilityService.getAllClosedFacilitiesByType(this.clientId, 'BG')
        .then(response => {
        this.accounts.splice(0, this.accounts.length)
        this.accounts = response;
        this.loading = false;
