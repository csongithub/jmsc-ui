...
export default {
    savePayment(payment) {
        return api.post( '/v2/payment/save/', payment).then(response => {
            let payment = response.data;
            return payment;
        }).catch(err => {
            console.log("Error while saving payment: " + JSON.stringify(err));
            return Promise.reject(err);
        });
    },
    getPayment(client_id, payment_id) {
        return api.get( '/v2/payment/get/'+ client_id + '/' + payment_id).then(response => {
            let payment = response.data;
            return payment;
        }).catch(err => {
            console.log("Error while saving payment: " + JSON.stringify(err));
    ...
    getAllDrafts(client_id, status) {
        return api.get( '/v2/payment/all_drafts/' +  client_id + '/' + status).then(response => {
    ...
    deletePayment(client_id, payment_id, status) {
        return api.delete( '/v2/payment/delete/' +  client_id + '/' + payment_id + '/' + status).then(response => {
    ...
    paymentsBetweenDates(client_id, status, req) {
        ...
        return api.put('/v2/payment/payment_between_dates?' + params, req).then(response => {
